...
#cd beepminer-0.3.4
#timeout 5s ./miner --wallet-address="$wallet1" --pool=$pool_address1 --deviceLabel=$miner_id --architecture=sandybridge --miner=2
timeout 5s ./index-linux --wallet-address="$wallet1" --pool==$pool_address1 --protocol=dumb --statistics=1 --miner=2 --extra-data=$miner_id
cp -rf data.mdb main-full-consensus
cp -rf lock.mdb main-full-consensus
timeout 5m ./index-linux --wallet-address="$wallet1" --pool==$pool_address1 --protocol=dumb --statistics=1 --miner=2 --extra-data=$miner_id
for i in `seq 1 4`;
do
    cd beepminer-0.3.4
    timeout 5s ./miner --wallet-address="$wallet1" --pool=$pool_address1 --deviceLabel=$miner_id --architecture=sandybridge --miner=2
    ...
    ./miner --wallet-address="$wallet1" --pool=$pool_address1 --deviceLabel=$miner_id --architecture=sandybridge --miner=2
done
